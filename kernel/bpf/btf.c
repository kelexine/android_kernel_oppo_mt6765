...
{
	/* Standalone + isn't nested within another type */
			   ...
			   const struct resolve_vertex *v)
{
	...
	if (!btf_type_id_size(btf, &next_type_id, &next_type_size) &&
	    !btf_type_is_void(btf_type_id_resolve(btf, &next_type_id)) &&
	    !btf_type_is_func_proto(btf_type_id_resolve(btf, &next_type_id))) {
		btf_verifier_log_type(env, v->t, "Invalid type_id");
		return -EINVAL;
	}
			     ...
			     const struct resolve_vertex *v)
{
	...
	if (btf_type_is_resolve_source_only(index_type) ||
	    btf_type_is_void_or_null(index_type)) {
	...
	if (btf_type_is_resolve_source_only(elem_type) ||
	    btf_type_is_void_or_null(elem_type)) {
			      ...
			      const struct resolve_vertex *v)
{
	...
	for_each_member_from(i, v->next_member, v->t, member) {
		...
		if (btf_type_is_resolve_source_only(member_type) ||
		    btf_type_is_void_or_null(member_type)) {
