			    ...
			    u8 opcode, bool is_jmp32)
{
	...
	case BPF_JNE:
	{
		...
		/* If the register is false_reg, then we're reaching this state
		 * because the branch was NOT taken. If it was a JNE, then that
		 * means that the compare succeeded, and we can info about the
		 * register.
		 * If this was a JEQ, then the compare failed, but we still know
		 * that the register doesn't have the value. We could possibly
		 * deduce some info about the value from that (if it was a
		 * constant), but we don't bother for now.
		 * If the register is true_reg, then we're reaching this state
		 * because the branch WAS taken. If it was a JEQ, that means
		 * the compare succeeded, and we can info about the register.
		 * If it was a JNE, then the compare failed, but we know nothing
			  ...
			  union bpf_attr __user *uattr)
{
	...
	for (i = 0; i < nfuncs; i++) {
		...
		if (i == 0)
			prog->aux->type_id = krecord[i].type_id;
		env->subprog_info[i].type_id = krecord[i].type_id;
