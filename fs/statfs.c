...
#include "mount.h"
#endif // #if defined(CONFIG_MYSU_MYSUFS_SUS_MOUNT) || defined(CONFIG_MYSU_MYSUFS_OPEN_REDIRECT)
#include "internal.h"
...
extern bool mysufs_is_inode_sus_kstat(struct inode *inode, bool *out_is_fuse);
extern int mysufs_sus_kstat_spoof_vfs_statfs(struct inode *inode, struct kstatfs *buf, bool *is_fuse);
...
#ifdef CONFIG_MYSU_MYSUFS_SUS_KSTAT
static int mysufs_statfs_by_dentry(struct dentry *dentry, struct kstatfs *buf, bool *is_fuse)
{
	int retval;
	...
	if (retval)
		return retval;
	if (mysufs_sus_kstat_spoof_vfs_statfs(d_backing_inode(dentry), buf, is_fuse))
...
int vfs_statfs(const struct path *path, struct kstatfs *buf)
{
	int error;
#ifdef CONFIG_MYSU_MYSUFS_SUS_KSTAT
	if (mysufs_is_current_app_uid()) {
		struct inode *inode = d_backing_inode(path->dentry);
		bool is_fuse = false;
		if (mysufs_is_inode_sus_kstat(inode, &is_fuse)) {
			// - here we do not call calculate_f_flags() as buf->f_flags will be spoofed
			//   by mysufs_statfs_by_dentry().
			return mysufs_statfs_by_dentry(path->dentry, buf, &is_fuse);
		}
	}
#endif // #ifdef CONFIG_MYSU_MYSUFS_SUS_KSTAT

#ifdef CONFIG_MYSU_MYSUFS_SUS_MOUNT
	if (likely(mysufs_is_current_proc_umounted())) {
