...
#if defined(CONFIG_MYSU_MYSUFS_SUS_KSTAT) || defined(CONFIG_MYSU_MYSUFS_SUS_MAP) || defined(CONFIG_MYSU_MYSUFS_OPEN_REDIRECT)
#include <linux/mysufs_def.h>
#endif // #if defined(CONFIG_MYSU_MYSUFS_SUS_KSTAT) || defined(CONFIG_MYSU_MYSUFS_SUS_MAP) || defined(CONFIG_MYSU_MYSUFS_OPEN_REDIRECT)
...
extern void mysufs_sus_kstat_spoof_show_map_vma(struct inode *inode, dev_t *out_dev, unsigned long *out_ino);
#endif // #ifdef CONFIG_MYSU_MYSUFS_SUS_KSTAT
#ifdef CONFIG_MYSU_MYSUFS_OPEN_REDIRECT
extern struct srcu_struct mysufs_srcu_open_redirect;
extern int mysufs_open_redirect_spoof_show_map_vma_srcu(struct inode *inode, unsigned long *out_ino, dev_t *out_dev, char **out_spoofed_name);
...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	if (file) {
		...
		if (MYSUFS_IS_INODE_OPEN_REDIRECT(inode)) {
			...
			int ret = mysufs_open_redirect_spoof_show_map_vma_srcu(inode, &ino, &dev, &spoofed_redirected_name);
			if (!ret) {
		...
		if (MYSUFS_IS_INODE_SUS_MAP(inode))
			return;
		...
		pgoff = ((loff_t)vma->vm_pgoff) << PAGE_SHIFT;
#ifdef CONFIG_MYSU_MYSUFS_SUS_KSTAT
		mysufs_sus_kstat_spoof_show_map_vma(inode, &dev, &ino);
#endif // #ifdef CONFIG_MYSU_MYSUFS_SUS_KSTAT
	}
			    ...
			    size_t count, loff_t *ppos)
{
	...
	while (count && (start_vaddr < end_vaddr)) {
		...
		if (vma && vma->vm_file && MYSUFS_IS_INODE_SUS_MAP(file_inode(vma->vm_file)))
			goto bypass_orig_flow;
#endif // #ifdef CONFIG_MYSU_MYSUFS_SUS_MAP
