...
#if defined(CONFIG_MYSU_MYSUFS_SUS_PATH) || defined(CONFIG_MYSU_MYSUFS_OPEN_REDIRECT)
...
extern bool mysufs_is_inode_sus_path(struct inode *inode);
extern const struct qstr mysufs_fake_qstr_name;
#endif
#ifdef CONFIG_MYSU_MYSUFS_OPEN_REDIRECT
extern struct filename *mysufs_open_redirect_spoof_do_sys_openat(struct inode *inode);
...
#ifdef CONFIG_MYSU_MYSUFS_SUS_PATH
	if (dentry && !IS_ERR(dentry) && dentry->d_inode && mysufs_is_inode_sus_path(dentry->d_inode)) {
...
#ifdef CONFIG_MYSU_MYSUFS_SUS_PATH
	if (unlikely(dentry) && !IS_ERR(dentry) && dentry->d_inode && !found_sus_path && mysufs_is_inode_sus_path(dentry->d_inode)) {
		if (d_in_lookup(dentry))
			d_lookup_done(dentry);
		if (!(flags & LOOKUP_RCU))
			dput(dentry);
		...
		dentry = d_alloc(base, &mysufs_fake_qstr_name);
		found_sus_path = true;
		goto retry;
	}
#endif
		       ...
		       unsigned *seqp)
{
	...
	if (nd->flags & LOOKUP_RCU) {
		...
		if (is_nd_state_lookup_last_and_open_last && dentry && !IS_ERR(dentry) && dentry->d_inode &&
			mysufs_is_inode_sus_path(dentry->d_inode))
		{
			if (d_in_lookup(dentry))
				d_lookup_done(dentry);
...
#ifdef CONFIG_MYSU_MYSUFS_SUS_PATH
					if (found_sus_path) {
						dentry = d_alloc_parallel(dir, &mysufs_fake_qstr_name, &wq);
						goto retry;
					}
#endif
					goto again;
...
static int link_path_walk(const char *name, struct nameidata *nd)
{
	...
	for(;;) {
		...
		{
			struct dentry *dentry = nd->path.dentry;
			if (dentry->d_inode && mysufs_is_inode_sus_path(dentry->d_inode)) {
		...
		struct file *file)
{
	...
	if ((likely(!error) && old_dfd != -1) &&
		MYSUFS_IS_INODE_OPEN_REDIRECT_WITHOUT_UID_CHECK(path.dentry->d_inode))
	{
		fake_filename = mysufs_open_redirect_spoof_do_sys_openat(path.dentry->d_inode);
...
#ifdef CONFIG_MYSU_MYSUFS_OPEN_REDIRECT
		if (!error && old_dfd != -1 &&
			MYSUFS_IS_INODE_OPEN_REDIRECT_WITHOUT_UID_CHECK(nd->path.dentry->d_inode))
		{
			fake_filename = mysufs_open_redirect_spoof_do_sys_openat(nd->path.dentry->d_inode);
			if (fake_filename && !IS_ERR(fake_filename)) {
...
#ifdef CONFIG_MYSU_MYSUFS_OPEN_REDIRECT
extern int mysufs_open_redirect_spoof_vfs_readlink(struct inode *inode, char __user *buffer, int buflen);
#endif
...
#ifdef CONFIG_MYSU_MYSUFS_OPEN_REDIRECT
		{
			if (MYSUFS_IS_INODE_OPEN_REDIRECT(inode)) {
				res = mysufs_open_redirect_spoof_vfs_readlink(inode, buffer, buflen);
				if (!res)
					return res;
			}
