 ...
 * Filename:
 * ---------
 *	 limu_shinetech_front_s5k5e9yx04_mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "limu_shinetech_front_s5k5e9yx04_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = LIMU_SHINETECH_FRONT_S5K5E9YX04_SENSOR_ID,
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x0136, 0x18);
	write_cmos_sensor(0x0137, 0x00);
	write_cmos_sensor(0x0305, 0x08);
	write_cmos_sensor(0x0306, 0x00);
	write_cmos_sensor(0x0307, 0xB3);
	...
	write_cmos_sensor(0x0101, 0x00);
	write_cmos_sensor(0x0340, 0x07);
	write_cmos_sensor(0x0341, 0xCA);
	write_cmos_sensor(0x0342, 0x0B);
	write_cmos_sensor(0x0343, 0xB0);
	write_cmos_sensor(0x0200, 0x0B);
	write_cmos_sensor(0x0201, 0x24);
	write_cmos_sensor(0x0202, 0x00);
	write_cmos_sensor(0x0203, 0x02);
	write_cmos_sensor(0x30B8, 0x2E);
	write_cmos_sensor(0x30BA, 0x36);
	...
	while(imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			printk("wenhui s5k5e9yxmipiraw_Sensor get_imgsensor_id *sensor_id = 0x%x",*sensor_id);
			if (*sensor_id == LIMU_SHINETECH_FRONT_S5K5E9YX04_SENSOR_ID) {
				...
				*sensor_id = imgsensor_info.sensor_id;
				...
				return ERROR_NONE;
			printk("wenhui Read sensor id fail, id: 0x%x\n", *sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
UINT32 LIMU_SHINETECH_FRONT_S5K5E9YX04_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
